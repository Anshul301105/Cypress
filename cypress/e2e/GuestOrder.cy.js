...
describe('Guest Order Flow', function () {
    ...
    it('End to End Process for Guest Orders', function () {
        ...
        objProductRepo
            .lblProductPage()
            .should('have.text', this.exm.ProductPageTitle);
        ...
        objProductRepo
            ...
            .then((id) => {
                ...
                cy.request(
                    `https://www.youniqueproducts.com/myk/bulk/${selectedProductId}`
                ).then((res, a) => {
                    ...
                    cy.get('.reactSelect__menu-list .reactSelect__option ')
                        .first()
                        .click();
                    ...
                    cy.get('.pdpAddToCart')
                        ...
                        .should(
                            'have.text',
                            `Add to cart(${priceWithCurrency(selectedPPrice)})`
                        );
                    // 13
                    cy.get('.pdpAddToCart').click();

                    // 14
                    cy.get('[data-testid=mini-cart-items]')
                        .first()
                        .find('.pQty')
                        .should('have.text', '1');
                    cy.get('[data-testid=mini-cart-items]')
                        .first()
                        .find('.pName')
                        .should('have.text', selectedPName);
                    cy.get('[data-testid=mini-cart-items]')
                        .first()
                        .find('.pPrice')
                        .should('have.text', selectedPPrice);
                    ...
                    cy.get('[data-testid=mini-cart-items]')
                        ...
                        .should(
                            'have.text',
                            'YOUNIQUE DAILY·YOU liquid collagen shot sample'
                        );
                    cy.get('[data-testid=mini-cart-items]')
                        .eq(1)
                        .find('.pPrice')
                    ...
                    // 0024
                    cy.get('.shoppingCart .item-container')
                        .first()
                        .find('.itemName')
                        .should('have.text', selectedPName);
                    // 0025
                    cy.get('.shoppingCart .item-container')
                        .first()
                        .find('.item-sku')
                        .should('have.text', selectedPSKU);
                    //0026
                    cy.get('.shoppingCart .item-container')
                        .first()
                        .find('.receiptLineTotal')
                        .should('have.text', priceWithCurrency(selectedPPrice));
                    // 0027
                    cy.get('.shoppingCart .item-container')
                        .first()
                        .find('[data-testid=quantityInput]')
                        .should('have.value', selectProductQty);
                    //0028
                    cy.get('.shoppingCart .item-container')
                        .first()
                        .find('[data-testid=removeItemButton]')
                        .should('exist');

                    // footer

                    cy.get('.cart-total-items').should(
                        'have.text',
                        totalSelectedProducts
                    );
                    cy.get('.cartFooter .cart-subtotal').should(
                        'have.text',
                        priceWithCurrency(selectedPPrice)
                    ); //
                    cy.get('.cartFooter .tax').should('have.text', tax); //
                    cy.get(
                        ".cartFooter [data-testid='shippingDisplay']"
                    ).should('have.text', priceWithCurrency(shipping)); //
                    cy.get('#carttotal').should(
                        'have.text',
                        `${priceWithCurrency(
                            parseFloat(selectedPPrice) + shipping
                        )} USD`
                    );
                    ...
                    // // "Products/checkout"
                    cy.get('#orderItems .lineItem')
                        .first()
                        .find('.pQty')
                        .contains(selectProductQty);
                    cy.get('#orderItems .lineItem')
                        .first()
                        .find('.pName')
                        .contains(selectedPName);
                    cy.get('#orderItems .lineItem')
                        .first()
                        .find('.pAmount')
                        .contains(priceWithCurrency(selectedPPrice));
                    ...
                    cy.get('#totalSummaries .shippingFee').contains(
                        priceWithCurrency(shipping)
                    );
                    ...
                    cy.get('#addressSection .first_nameformError').should(
                        'exist'
                    );
                    cy.get('#first_name')
                        .focus()
                        .type(this.exm.shippingInformation.fName);

                    cy.get('#last_name').focus().blur();
                    cy.get('#addressSection .last_nameformError').should(
                        'exist'
                    );
                    cy.get('#last_name')
                        .focus()
                        .type(this.exm.shippingInformation.lName);

                    cy.get('#email').focus().blur();
                    cy.get('#addressSection .emailformError').should('exist');
                    cy.get('#email')
                        .focus()
                        .type(this.exm.shippingInformation.email);

                    cy.get('#phone').focus().blur();
                    cy.get('#addressSection .phoneformError').should('exist');
                    cy.get('#phone')
                        .focus()
                        .type(this.exm.shippingInformation.phone);

                    cy.get('#address1').focus().blur();
                    cy.get('#addressSection .address1formError').should(
                        'exist'
                    );
                    cy.get('#address1')
                        .focus()
                        .type(this.exm.shippingInformation.address1);

                    cy.get('#city')
                        .focus()
                        .type(this.exm.shippingInformation.city);
                    cy.get('#state').select(this.exm.shippingInformation.state);

                    cy.get('#postalcodelookup').focus().blur();
                    cy.get('#addressSection .postalcodelookupformError').should(
                        'exist'
                    );
                    cy.get('#postalcodelookup')
                        .focus()
                        .type(this.exm.shippingInformation.zipcode);

                    cy.intercept(
                        'https://stagingbeta.youniqueproducts.com/cer/carts/us/active/calculate'
                    ).as('calculateCall');
                    ...
                    cy.getIFrame('#braintree-hosted-field-number')
                        ...
                        .type(this.exm.shippingInformation.creditCardNo);
                    cy.getIFrame('#braintree-hosted-field-expirationDate')
                        .find('#expiration')
                        .type(this.exm.shippingInformation.exp);
                    cy.getIFrame('#braintree-hosted-field-cvv')
                        .find('#cvv')
                        .type(this.exm.shippingInformation.cvv);

                    cy.wait(3000);
                    cy.get(
                        '.continue span[data-original-text="Continue"]'
                    ).click();
                    cy.intercept(
                        'https://stagingbeta.youniqueproducts.com/api/shoppingcart/purchase'
                    ).as('purchaseCall');
                    cy.get('#submit-order').click();
                    cy.wait('@purchaseCall');
                    cy.wait(10000);
                    cy.url().should('include', this.exm.recieptRoute);
                    cy.get('.cartTable tbody')
                        .first()
                        .find('.td-product a')
                        .contains(selectedPName);
                    cy.get('.cartTable tbody')
                    ...
                    cy.get('.cartTable tbody')
                        .first()
                        .find('.td-product .sku')
                        .contains(selectedPSKU);
                    cy.get('.subTotal').contains(selectedPPrice);
                    cy.get('.shippingTotal').contains(
                        priceWithCurrency(shipping)
                    );
