...
const objRepo = new repo()
    ...
    it('Register UnSuccessful - Empty Mandatory fields', function () {
        ...
        objRepo.lblFirstNameRequired().should('have.text', "First name is required")
        objRepo.lblLastNameRequired().should('have.text', "Last name is required")
        objRepo.lblPasswordRequired().contains("Password is required")
        objRepo.lblConfirmPasswordRequired().should('have.text', "Verify Password is required")
        })
    })

   it.only('Register Successful and Navigate to Dashboard', function () {
      ...
      objRepo.btnGetStarted().click()
         ...
         cy.registerEnterVerifyPassword(this.registerData.EnterVerifyPassword)
    })
    cy.registerClickCreateAccountButton()
    cy.get('.profileName .name').should('have.text', `You are registering as ${this.registerData.EnterFirstName} ${this.registerData.EnterLastName}!`)

    objRepo.lblSelectCountry().contains("Where do you live?");
    objRepo.lblFancyCountry().click();
    cy.get('.fancyDropdownMenu li').eq(0).click();
 ...
 cy.wait(3000)
    cy.intercept('https://stagingbeta.youniqueproducts.com/search/v1/presenter').as('interceptCall')
    ...
    cy.intercept('api/presenter/checkSiteAvailable/lips').as('checkAvailability');
    ...
    cy.get('#siteName').clear().type(`${this.registerData.EnterFirstName}${this.registerData.EnterLastName}`);
    cy.intercept(`api/presenter/checkSiteAvailable/${this.registerData.EnterFirstName}${this.registerData.EnterLastName}`).as('checkAvailabilityAgain');
    cy.get('.checkSite ').click();
    ...
    cy.getIFrame('#braintree-hosted-field-number').find("#credit-card-number").type(this.guestOrderData.shippingInformation.creditCardNo);
                cy.getIFrame('#braintree-hosted-field-expirationDate').find("#expiration").type(this.guestOrderData.shippingInformation.exp);
                cy.getIFrame('#braintree-hosted-field-cvv').find("#cvv").type(this.guestOrderData.shippingInformation.cvv);
...
cy.wait(5000);
cy.get('[data-testid="presenterName"]').contains(`Welcome to the Younique Family, ${this.registerData.EnterFirstName} ${this.registerData.EnterLastName}!`)
   })
})